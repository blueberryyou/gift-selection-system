...
- 🔓 **无障碍访问**：打开页面即可直接浏览和选择礼物，无需登录
- 🔐 **管理员保护**：管理后台需要密码验证（管理员密码：`xes2026`）
...
在代码中找到并修改管理员密码（约第 620 行）：

```javascript
const ADMIN_PASSWORD = "xes2026"; // 修改为您的管理员密码
```
...
1. 打开网页，直接进入礼物选择页面（无需密码）
2. 在"选择礼物"页面浏览可用礼物和剩余库存
3. 点击"选择此礼物"按钮
4. 填写个人信息（姓名、达标时间）
5. 上传达标截图作为凭证
6. 确认提交
...
管理员可以在"管理后台"页面（需要密码 `xes2026`）：
- 查看实时库存状态
- 重置库存数量
- 清空选择记录
- 重置全部数据

**首次访问管理后台**：点击"管理后台"标签时会弹出密码验证框，输入正确密码后即可进入。

## 🔒 安全建议

1. **修改管理员密码**：部署前请务必在代码中修改默认管理员密码
2. **配置 Firebase 规则**：在生产环境使用更严格的数据库访问规则
3. **HTTPS 部署**：确保网站通过 HTTPS 访问
4. **定期备份**：定期导出和备份选择记录
5. **保护管理员密码**：不要在公开场合分享管理员密码
